...
async function contracts_payment_calculate(contractId) 
{
 ...
 // let cstmoment = moment(cst);
  ...
  if (is_ft == 'no')
  { 
    let contractMonth1 = moment(ced).diff(moment(cst), 'months');
    let circletimes1 = contractMonth1/pc;
    for (let i=0;i<=circletimes1;i++)
    {
      let initialst = i*pc;
      let initialet = (i+1)*pc;
      let ccstmoment = moment(cst).add(initialst, 'M');
      let ccedmoment = moment(cst).add(initialet, 'M').add(-.1, 'd');
